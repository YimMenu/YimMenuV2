...
#include "types/ped/PedConfigFlag.hpp"  // Added explicit include for PedConfigFlag
	...
	class UnlockAll : public Command
	{
		...
		void unlockWeapons()
		{
			...
			for (auto weapon : Data::Weapons)
			{
				...
				WEAPON::SET_PED_WEAPON_TINT_INDEX(Self::GetPed().GetHandle(), weapon, 7); // Set maximum tint
			}

			// Unlock weapon attachments/mods
			...
			// Special Weapons
			Stats::SetInt("MPX_REV_NV_KILLS", 50); // Navy Revolver Kills
			
			// Weapon Specific Unlocks from LUA script
			loopBool(15425, 15439); // Weapon mods including Knuckleduster, Technical Custom, Minigun etc.
			loopBool(51189, 51189); // Spray Can
			loopBool(51196, 51197); // The Shocker, Bottom Dollar Bail Enforcement tint for Stungun
		}
		...
		void unlockVehicles()
		{
			...
			// Special Vehicles
			loopBool(25407, 25511); // Arena War Vehicles (Slamvan variants, Brutus mods, etc.)
			loopBool(25516, 25516); // RC Tank
			loopBool(25520, 25521); // Metal Detector
			
			// Acid Lab Unlocks
			Stats::SetInt("MPX_XM22_FLOW", -1); 
			Stats::SetInt("MPX_XM22_MISSIONS", -1);
			Stats::SetInt("MPX_AWD_CALLME", 50); // Acid Lab Equipment Unlock
		}
		...
		void unlockAchievements()
		{
			...
			// Collectibles
			loopBool(26811, 26964); // Action Figures, Playing Cards
			loopBool(30230, 30251); // Movie Props, Space Interloper Outfit
		}
		...
		void unlockHeists()
		{
			...
			Stats::SetInt("MPX_H3_VEHICLESUSED", -1); // Trade Price for Diamond Casino Heist Finale
		...
		void unlockClothesHairstyles()
		{
			...
			// Special T-Shirts and Outfits
			loopBool(4247, 4269); // Various hats and shirts like 'Magnetics Script' Hat, 'Zombie Liberals In The Midwest' T-shirt
			loopBool(31760, 31764); // Faces of Death T-Shirt, Straight to Video T-Shirt, etc
			loopBool(31766, 31777); // Sprunk Cap, eCola Cap, branded T-Shirts and Coveralls
			loopBool(28254, 28255); // Get Metal T-Shirt / Axe of Fury T-Shirt
			loopBool(42286, 42287); // Ludendorff Survivor, Pizza This outfits and caps
			...
			loopBool(32312, 32312); // Agency Computer Tutorial
			
			Stats::SetBool("MPX_CARMEET_PV_CHLLGE_CMPLT", true); // Car Meet Challenge
		}
		...
		void unlockTattoos()
		{
			...
			// Additional tattoo unlocks from the LUA script
		...
		void unlockPedConfigFlags()
		{
			...
			{
				// Set beneficial ped config flags
		...
		{
			// Main packed bool unlocks (more comprehensive range from LUA script)
			loopBool(0, 500);    // First 500 packed bools
			loopBool(15425, 15439); // Weapon unlocks
			loopBool(25407, 25511); // Arena War vehicles
			loopBool(25516, 25521); // RC Tank and Metal Detector
			loopBool(26811, 26964); // Collectibles
			loopBool(28254, 28255); // Special T-shirts
			loopBool(30230, 30251); // Movie Props
			loopBool(31760, 31777); // Various clothing items
			loopBool(32312, 32312); // Agency Computer Tutorial
			loopBool(42286, 42287); // Ludendorff outfits
			loopBool(51189, 51189); // Spray Can
			loopBool(51196, 51197); // Stungun variants
		}
		
		void unlockLifetimeBuys()
		{
			// Set lifetime buys for certain unlocks
