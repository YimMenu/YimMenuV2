...
namespace YimMenu
{
	...
	bool Pointers::Init()
	{
		...
		constexpr auto sendEventAckPtrn = Pattern<"84 C0 75 ? 89 EE 89 F0 48 83 C4">("SendEventAck");
		scanner.Add(sendEventAckPtrn, [this](PointerCalculator ptr) {
			SendEventAck = ptr.Sub(0x174).As<Functions::SendEventAck>();
			// EventAck was a relative call above this site; it's call rbx now
			EventAck = nullptr;
		});
		...
		scanner.Add(battlEyeStatusUpdatePatchPtrn, [this](PointerCalculator ptr) {
			// pattern still hits the arxan mid-site, but early-ret there crashes now.
			// real entry is +0x10C (push rbp / sub rsp, 0xA0), just ret out of the call.
			auto entry = ptr.Add(0x10C).As<std::uint8_t*>();
			if (entry && entry[0] == 0x55 && entry[1] == 0x48 && entry[2] == 0x81)
			{
				BattlEyeStatusUpdatePatch = BytePatches::Add(entry,
		...
		// old sig died; B0 01 50 48 8D 05... is an arxan stub into the middle.
		// this is the real entry (rcx=session, rdx=info, ...)
